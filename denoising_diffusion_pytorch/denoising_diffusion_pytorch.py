...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step'):
            ...
            # plt.show()
            # plt.close()
            # print(f"{img.min():.2f}, {img.max():.2f}, {img.mean():.2f}")
            img = self.p_sample(img, t)

        img = unnormalize_cifar10(img)
        return img
    ...
    def forward(self, img, *args, **kwargs):
        ...
        # img = normalize_to_neg_one_to_one(img)  # image is already normalized from the loader
...
class Trainer(object):
    def __init__(
        ...
        assert has_int_squareroot(args.num_samples), 'number of samples must have an integer square root'
    ...
    def load(self, milestone):
        data = torch.load(str(self.results_folder / f'models/model-{milestone}.pt'))
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss, model_out = self.model(data)
                        loss = loss / self.gradient_accumulate_every
                        total_loss += loss.item()
                        total_model_out += torch.linalg.norm(model_out, dim=(1,2,3)).mean().item()

                    self.accelerator.backward(loss)
                log_dict["noise_pred"] = total_model_out / self.gradient_accumulate_every
                log_dict["loss"] = total_loss
                log_dict["lr"] = self.opt.param_groups[0]["lr"]
                log_dict["wd"] = self.opt.param_groups[0]["weight_decay"]
