...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=1, embed_dim=768, depth=12,
        ...
        # self.t_embed = nn.Sequential(nn.Linear(1, embed_dim//4), nn.ReLU(), nn.Linear(embed_dim//4, embed_dim))
        ...
        self.patch_size = patch_size
        # self.heads = nn.ModuleList([nn.Linear(embed_dim, 3*patch_size**2) for _ in range(num_patches)])
    ...
    def prepare_tokens(self, x):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)  # patch linear embedding, [bs, num_patches, embed_dim]
    ...
    def forward(self, x):
        ...
        x = x + pos_embed  # add positional embedding again before the projection
