...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=1,
        ...
        # self.t_embed = nn.Sequential(nn.Linear(1, embed_dim//4), nn.ReLU(),
        ...
        dpr = [x.item() for x in
               torch.linspace(0, drop_path_rate, depth)]  # stochastic depth
        # decay rule
        ...
        # else nn.Identity()
        self.patch_size = patch_size
        # self.heads = nn.ModuleList([nn.Linear(embed_dim, 3*patch_size**2)
    ...
    def prepare_tokens(self, x):
        B, nc, w, h = x.shape
        x = self.patch_embed(
            x)  # patch linear embedding, [bs, num_patches, embed_dim]
...
def vit_tiny(patch_size=16, **kwargs):
    model = VisionTransformer(patch_size=patch_size, embed_dim=192, depth=12,
